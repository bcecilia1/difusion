...
using Plots,DifferentialEquations,StatsBase
...
en un dominio circular (el disco unitario) con condiciones de borde de Neumann homogéneas. Pensaremos a $u$ como la concentración de una cierta substancia (azúcar o tinta) en un líquido (café con leche o agua). La substancia tenderá a disolverse en el líquido gracias al término difusivo. 
...
md"""
Pasamos a coordenadas polares quedándonos:

$u_t = \alpha\Big(u_{rr} + \frac{1}{r}u_r + \frac{1}{r^2}u_{\theta\theta}\Big) + \beta u_\theta.$

y resolvemos esta ecuación en el rectángulo $(r,\theta)\in[0,1]\times[0,2\pi)$. 
"""
...
Escribimos la discretización de las ecuaciones e implementamos la función que corresponde al sistema para generar un ODEProblem. """
...
function sistema!(du, u, p, t)
	...
	du[1, 1] = α*((u[2, 1] - 2*u[1, 1] + u[2, 1])/(Δr^2)) 
	du[1, J] = α*((u[2, 1] - 2*u[1, 1] + u[2, 1])/(Δr^2))
	du[I, 1] = α*((u[I - 1, 1] - 2*u[I, 1] + u[I - 1, 1])/(Δr^2) + (1/r[I]^2)*(u[I, 2] -2*u[I, 1] + u[I, J - 1])/(Δθ^2)) + β*(u[I, 2] - u[I, J - 1])/(2*Δθ)
	du[I, J] = α*((u[I - 1, J] - 2*u[I, J] + u[I - 1, J])/(Δr^2) + (1/r[I]^2)*(u[I, 2] -2*u[I, J] + u[I, J - 1])/(Δθ^2)) + β*(u[I, 2] - u[I, J - 1])/(2Δθ) 
	for i in 2:(I-1)
		for j in 2:(J-1)
			du[i, j] = α*((u[i + 1, j] - 2*u[i, j] + u[i - 1, j])/(Δr^2) + (1/r[i])*(u[i+1, j] - u[i-1, j])/(2Δr) + (1/(r[i]^2))*(u[i, j + 1] -2*u[i, j] + u[i, j - 1])/(Δθ^2)) + β*(u[i, j + 1] - u[i, j - 1])/(2Δθ)
		end
	end
	# Condición 1 (de borde)
	for j in 2:(J-1)
		du[I, j] = α*((2*u[I - 1, j] - 2*u[I, j])/(Δr^2) + (1/(r[I]^2))*(u[I, j + 1] -2*u[I, j] + u[I, j - 1])/(Δθ^2)) + β*(u[I, j + 1] - u[I, j - 1])/(2Δθ)
	end
	# Condición 2 (de borde)
	for i in 2:(I-1)
		du[i, 1] = α*((u[i+1, 1] - 2*u[i, 1] + u[i-1, 1])/(Δr^2) + (1/r[i]^2)*(u[i, 2] -2*u[i, 1] + u[i, J-1])/(Δθ^2) + (1/r[i])*(u[i+1, 1] - u[i-1, 1])/(2Δr)) + β*(u[i, 2] - u[i, J-1])/(2Δθ)
	end
	# Condición 3 (de borde)
	for j in 2:(J-1)
		du[1, j] = α*((u[2, j] - 2*u[1, j] + u[2, j])/(Δr^2))
	end 
	# Condición 4 (de borde)
	for i in 2:(I-1) 
		du[i, J] = α*((u[i + 1, J] - 2*u[i, J] + u[i - 1, J])/(Δr^2) + (1/r[i])*(u[i+1, J] - u[i-1, J])/(2Δr) + (1/(r[i]^2))*(u[i, 2] -2*u[i, J] + u[i, J - 1])/(Δθ^2)) + β*(u[i, 2] - u[i, J - 1])/(2Δθ)
	end
end

# ╔═╡ 6109143c-aa01-48c0-bf1c-3d0f77f82b29
function resolver(r, Δr, Δθ, α, β, t, u₀)
	p = [r, Δr, Δθ, α, β]
	problema = ODEProblem(sistema!, u₀, t, p)
...
function pelicula(solucion, tiempos, Δθ, Δr, r)
	grillaθ = 0:Δθ:2π
	@gif for j in 1:(length(tiempos))
		heatmap(grillaθ, r, solucion(tiempos[j]), projection=:polar, clim = (0.0, 0.05))
	end
end
...
Resolvemos para $\alpha = 0.05$ y $\beta=3$. 
"""

# ╔═╡ b95b6a8f-67c1-43c5-a8ec-915769a6b484
begin
	α = 0.05
	β = 3
	Δr = 0.02
	...
	Δθ = 0.075
	t = (0, 20)
	u₀ = fill(0.0, (length(r), length(0:Δθ:2π)))
	u₀[26:35, 26:35] .= fill(1.0, (10, 10))
	sol = resolver(r, Δr, Δθ, α, β, t, u₀)
end

# ╔═╡ f8665267-b299-4008-8a2f-80c1bb7085c4
pelicula(sol, 0:0.1:20, Δθ, Δr, r)

# ╔═╡ 84d8d1da-3442-4a0b-9008-1df3a5fe3292
md"""Probamos con otros valores de α y β"""

# ╔═╡ adb3121c-a6c7-4fe0-817b-edddd1da7864
begin
	α₁ = 0.25
	β₁ = 5
	sol₁ = resolver(r, Δr, Δθ, α₁, β₁, t, u₀)
...
# ╔═╡ 97a54b2a-af04-408e-a24b-7c639604d63b
begin
	α₂ = 0.01
	β₂ = 2
	sol₂ = resolver(r, Δr, Δθ, α₂, β₂, t, u₀)
...
# ╔═╡ eb5acfb3-dc51-4db0-943a-f68f0ef6582d
md"""Al cambiar los valores de los parámetros cambia la rapidez con la que se homogeniza la temperatura. Con α y β más altos, se homogeniza más rápido mientras que con α y β bajos no llega a hacerlo en los 20 segundos."""

# ╔═╡ 7aeb4017-f55f-49db-8019-072f36b50634
md"""
###### Extra: 

Modificamos las ecuaciones para permitir considerar campos de velocidades que varíen a lo largo de todo el dominio. 
"""

# ╔═╡ 6815e97b-28f4-4cc5-9196-325df8dae649
function sistema_βmatriz!(du, u, p, t)
	r, Δr, Δθ, α, β = p
	I = length(r) 
	J = length(0:Δθ:2π)
	# Construimos la matriz
	du[1, 1] = α*((u[2, 1] - 2*u[1, 1] + u[2, 1])/(Δr^2)) 
	du[1, J] = α*((u[2, 1] - 2*u[1, 1] + u[2, 1])/(Δr^2))
	du[I, 1] = α*((u[I - 1, 1] - 2*u[I, 1] + u[I - 1, 1])/(Δr^2) + (1/r[I]^2)*(u[I, 2] -2*u[I, 1] + u[I, J - 1])/(Δθ^2)) + β[I,1]*(u[I, 2] - u[I, J - 1])/(2*Δθ)
	du[I, J] = α*((u[I - 1, J] - 2*u[I, J] + u[I - 1, J])/(Δr^2) + (1/r[I]^2)*(u[I, 2] -2*u[I, J] + u[I, J - 1])/(Δθ^2)) + β[I,J]*(u[I, 2] - u[I, J - 1])/(2Δθ)
	for i in 2:(I-1)
		for j in 2:(J-1)
			du[i, j] = α*((u[i + 1, j] - 2*u[i, j] + u[i - 1, j])/(Δr^2) + (1/r[i])*(u[i+1, j] - u[i-1, j])/(2Δr) + (1/(r[i]^2))*(u[i, j + 1] -2*u[i, j] + u[i, j - 1])/(Δθ^2)) + β[i,j]*(u[i, j + 1] - u[i, j - 1])/(2Δθ)
		end
	end
	# Condición 1
	for j in 2:(J-1)
		du[I, j] = α*((2*u[I - 1, j] - 2*u[I, j])/(Δr^2) + (1/(r[I]^2))*(u[I, j + 1] -2*u[I, j] + u[I, j - 1])/(Δθ^2)) + β[I,j]*(u[I, j + 1] - u[I, j - 1])/(2Δθ)
	end
	# Condición 2
	for i in 2:(I-1)
		du[i, 1] = α*((u[i+1, 1] - 2*u[i, 1] + u[i-1, 1])/(Δr^2) + (1/r[i]^2)*(u[i, 2] -2*u[i, 1] + u[i, J-1])/(Δθ^2) + (1/r[i])*(u[i+1, 1] - u[i-1, 1])/(2Δr)) + β[i,1]*(u[i, 2] - u[i, J-1])/(2Δθ)
	end
	# Condición 3
	for j in 2:(J-1)
		du[1, j] = α*((u[2, j] - 2*u[1, j] + u[2, j])/(Δr^2))
	...
	# Condición 4
	for i in 2:(I-1) 
		du[i, J] = α*((u[i + 1, J] - 2*u[i, J] + u[i - 1, J])/(Δr^2) + (1/r[i])*(u[i+1, J] - u[i-1, J])/(2Δr) + (1/(r[i]^2))*(u[i, 2] -2*u[i, J] + u[i, J - 1])/(Δθ^2)) + β[i,J]*(u[i, 2] - u[i, J - 1])/(2Δθ)
	end
end

# ╔═╡ be1e632d-2eb2-43f1-a732-5313e2fa9c42
function resolver_βmatriz(r, Δr, Δθ, α, β, t, u₀)
	p = [r, Δr, Δθ, α, β]
	problema = ODEProblem(sistema_βmatriz!, u₀, t, p)
	solucion = solve(problema)
	return solucion
end

# ╔═╡ 2a68c96c-b695-49d3-9eaf-d02f41d799a0
md"""Probamos con r:"""

# ╔═╡ 19055637-4d6d-4bd7-ab1e-2b81c66b77cd
begin
  β₃ = [r[i] for i in 1:length(r), j in 1:length(0:Δθ:2π)]
  sol₃ =  resolver_βmatriz(r, Δr, Δθ, α, β₃, t, u₀)
  pelicula(sol₃, 0:0.1:20, Δθ, Δr, r)
end

# ╔═╡ 1890859d-5b15-46c2-bf57-7e258d7c0770
md"""Probamos con r(1-r):"""

# ╔═╡ 0c0e9bd1-f3bd-41e0-8694-4eab5d4a8310
begin
  r_mod = r.*(ones(length(r))-r)
  β₄ = [r_mod[i] for i in 1:length(r_mod), j in 1:length(0:Δθ:2π)]
  sol₄ =  resolver_βmatriz(r, Δr, Δθ, α, β₄, t, u₀)
  pelicula(sol₄, 0:0.1:20, Δθ, Δr, r)
end

# ╔═╡ 26c61ad0-75d5-439f-8bfe-0824ee1a578c
md"""Se puede ver que al correr con la matriz β compuesta por r cambia hacia donde se difunde el calor. Sin embargo, parece homogenizarse a la misma temperatura. """

# ╔═╡ 7b23fc47-bf5a-4501-836a-5ef3ba77860d
begin
  column(j) = r*cos(Δθ*j)
  β₅ = hcat([column(j) for j=1:length(0:Δθ:2π)]...)
  sol₅ =  resolver_βmatriz(r, Δr, Δθ, α, β₅, t, u₀)
  pelicula(sol₅, 0:0.1:20, Δθ, Δr, r)
end

# ╔═╡ bb018e67-5716-4913-bc15-5d6d1a7b1d66
begin
  column1(j) = r_mod*cos(Δθ*j)
  β₆ = hcat([column1(j) for j=1:length(0:Δθ:2π)]...)
  sol₆ =  resolver_βmatriz(r, Δr, Δθ, α, β₆, t, u₀)
  pelicula(sol₆, 0:0.1:20, Δθ, Δr, r)
end

# ╔═╡ 6706b224-0094-4a25-a632-00f3d957121b
md"""Se puede ver que en estos casos, con variaciones en θ, se difunde mucho más rápido. Además, se homogeniza a una temperatura mayor. """
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "4dfe8fe42c89e03fdc802454bba63e1d25882eda"
